...
from validation import validate_deferment
...
render_metric_row([
    {'label': f'{t("per_payment")} ({freq_label})', 'value': f'¥{payment:,.0f}'},
    {'label': t('annual_premium'), 'value': f'¥{ap:,.0f}'},
    {'label': t('lump_sum'), 'value': f'¥{lump:,.0f}', 'help': t('lump_sum_help')},
    {
        'label': f'{t("per_payout")} ({payout_label})',
        'value': f'¥{per_payout:,.0f}',
...
with section_panel(t('phase_title')):
