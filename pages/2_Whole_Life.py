...
from validation import validate_issue_age
...
T = {
    'zh': {
        ...
        'freq_section': '缴费方式',
        'term_auto': '保险期限 = 极限年龄 − 年龄 = {term} 年（自动）',
        ...
        'formula_title': '精算原理',
        'formula_1': r'''P = S \cdot \frac{A_x}{\ddot{a}_x}''',
        'formula_explain': '终身寿险净保费 = 保额 × 终身死亡给付期望现值 ÷ 终身年金现值',
        'formula_detail': '与定期寿险逻辑相同，但保障至极限年龄。准备金逐年上升，最终趋近保额。',
        ...
        'caption_summary': '{freq}缴纳 · 费用附加 +{extra}（α/β/γ）· 保障至 {max_age} 岁',
    ...
    'en': {
        ...
        'term_auto': 'Policy Term = Limit Age − Age = {term} years (auto)',
        ...
        'formula_1': r'''P = S \cdot \frac{A_x}{\ddot{a}_x}''',
        'formula_explain': 'Whole life net premium = Sum Insured × EPV of lifetime death benefit ÷ Life annuity-due',
        'formula_detail': 'Same logic as term life, but coverage extends to the limit age. Reserves increase monotonically, approaching the sum insured.',
    ...
    'zh-Hant': {
        ...
        'caption': '不論何時死亡皆給付 · 保障終身 · Net Premiums · Reserves',
        ...
        'term_auto': '保險期限 = 極限年齡 − 年齡 = {term} 年（自動）',
        ...
        'footer': '{age}歲 {gender} · 保額¥{sum:,} · 終身 · 利率{rate:.1%} · {risk} · {payment}',
        'formula_title': '精算原理',
        'formula_1': r'''P = S \cdot \frac{A_x}{\ddot{a}_x}''',
        'formula_explain': '終身壽險純保費 = 保額 × 終身死亡給付期望現值 ÷ 終身年金現值',
        'formula_detail': '與定期壽險邏輯相同，但保障至極限年齡。準備金逐年上升，最終趨近保額。',
...
<a href="/Whole_Life" target="_self" class="nav-pill" style="color:#1D1D1F;border-color:#AEAEB2;">{t('nav_whole_life')}</a>
...
from premium import gross_annual_premium, periodic_premium, whole_life_annual_premium as net_wl_ap
...
try:
    ...
    if use_ml:
        ...
        lt = cached_life_table(
            st.session_state.table_path,
            st.session_state.table_col,
            gender_code,
            risk_factor,
        )
...
st.caption(t('caption_summary', freq=freq_label, extra=f'¥{gap-net:,.0f}', max_age=max_age))
...
if all(v is not None for v in _report_vars):
    ...
    st.download_button("📥 下载报告 (HTML)", data=report_html,
                       file_name="2_Whole_Life_{{age}}_{{gender}}.html",
                       mime="text/html")

st.caption(t('footer', age=age, gender=gender, sum=sum_insured, rate=rate, risk=risk_label_display, payment=pay_label))
