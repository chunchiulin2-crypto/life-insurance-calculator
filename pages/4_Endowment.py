...
T = {
    'zh': {
        ...
        'pay_hint': '立即付款仅影响死亡赔付部分，到期返还不受影响',
        ...
        'reserve_chart': '责任准备金曲线（终点 = 保额）',
        'reserve_table': '各年末准备金明细',
        'col_year': '保单年度', 'col_reserve': '准备金',
        'footer': '{age}岁 {gender} · 保额¥{sum:,} · {term}年 · 利率{rate:.1%} · {risk} · {payment} · 到期返还',
        'formula_title': '精算原理',
        'formula_1': r'''A_{x:n} = A_{x:n}^{1} + {}_nE_x''',
        'formula_explain': '两全保险 = 定期寿险（死亡给付） + 纯生存保险（到期给付）',
        'formula_detail': '死亡则赔付保额；活到期满则返还保额。准备金从 0 出发，到期满时恰好等于保额。',
        ...
        'caption_summary': '{freq}缴纳 {term} 年 · 费用附加 +{extra} · 到期返还 ¥{sum_insured}',
        ...
        'ml_ai_note': 'AI 基于 GradientBoosting 预测，MAE<0.002，R²>0.99', 'ml_vs_traditional': 'AI 预测 vs 传统生命表',
        'uw_ai_label': '核保方式', 'uw_ai_manual': '人工分级', 'uw_ai_auto': 'AI 智能核保',
        'uw_ai_help': 'AI 基于健康数据自动评估风险因子',
        'uw_bmi_label': 'BMI', 'uw_bp_label': '血压 (收缩/舒张)', 'uw_cholesterol': '总胆固醇 (mg/dL)',
        'uw_family_history': '家族早逝史', 'uw_exercise': '运动频率', 'uw_alcohol': '饮酒',
        ...
        'uw_chronic': '慢性病史', 'uw_result_risk': '风险因子', 'uw_result_class': '核保等级', 'uw_result_mult': '费率乘数',
    ...
    'en': {
        ...
        'pay_hint': 'Acceleration only affects death benefit, not the maturity survival benefit',
        ...
        'reserve_chart': 'Policy Reserve Curve (ends at sum insured)',
        'reserve_table': 'Reserve by Policy Year',
        'col_year': 'Policy Year', 'col_reserve': 'Reserve',
        'footer': '{age}y {gender} · Sum ¥{sum:,} · {term}yr · Rate {rate:.1%} · {risk} · {payment} · Endowment',
        'formula_title': 'Actuarial Formula',
        'formula_1': r'''A_{x:n} = A_{x:n}^{1} + {}_nE_x''',
        'formula_explain': 'Endowment = Term Life (death benefit) + Pure Endowment (survival benefit)',
        'formula_detail': 'If death occurs, pays sum insured; if survival to maturity, returns sum insured. Reserve grows from 0 to exactly the sum insured at maturity.',
        ...
        'caption_summary': '{freq} × {term}yr · Loading +{extra} · Maturity ¥{sum_insured}',
        ...
        'ml_ai_note': 'AI via GradientBoosting, MAE<0.002, R²>0.99', 'ml_vs_traditional': 'AI Prediction vs Traditional Table',
        'uw_ai_label': 'Underwriting', 'uw_ai_manual': 'Manual', 'uw_ai_auto': 'AI Smart UW',
        'uw_ai_help': 'AI predicts risk factor from health data',
        'uw_bmi_label': 'BMI', 'uw_bp_label': 'BP (Sys/Dia)', 'uw_cholesterol': 'Total Cholesterol (mg/dL)',
        ...
        'uw_chronic': 'Chronic Condition', 'uw_result_risk': 'Risk Factor', 'uw_result_class': 'UW Class', 'uw_result_mult': 'Rate Multiplier',
        'uw_class_pref_plus': 'Preferred+', 'uw_class_standard': 'Standard', 'uw_class_substandard': 'Substandard', 'uw_class_decline': 'Decline',
    },
    'zh-Hant': {
        'title': '兩全保險 · Endowment',
        'caption': '死亡給付 + 到期返還 · Net Single & Annual Premiums · Policy Reserves',
        ...
        'reserve_chart': '責任準備金曲線（終點 = 保額）', 'reserve_table': '各年末準備金明細',
        'col_year': '保單年度', 'col_reserve': '準備金',
        'footer': '{age}歲 {gender} · 保額¥{sum:,} · {term}年 · 利率{rate:.1%} · {risk} · {payment} · 到期返還',
        'formula_title': '精算原理',
        'formula_1': r'''A_{x:n} = A_{x:n}^{1} + {}_nE_x''',
        'formula_explain': '兩全保險 = 定期壽險（死亡給付） + 純生存保險（到期給付）',
        'formula_detail': '死亡則給付保額；活到期滿則返還保額。準備金從 0 出發，到期滿時恰好等於保額。',
        ...
        'caption_summary': '{freq}繳納 {term} 年 · 費用附加 +{extra} · 到期返還 ¥{sum_insured}',
        ...
        'uw_bmi_label': 'BMI', 'uw_bp_label': '血壓 (收縮/舒張)', 'uw_cholesterol': '總膽固醇 (mg/dL)',
        'uw_family_history': '家族早逝史', 'uw_exercise': '運動頻率', 'uw_alcohol': '飲酒',
        ...
        'uw_chronic': '慢性病史', 'uw_result_risk': '風險因子', 'uw_result_class': '核保等級', 'uw_result_mult': '費率乘數',
...
<a href="/Endowment" target="_self" class="nav-pill" style="color:#1D1D1F;border-color:#AEAEB2;">{t('nav_endowment')}</a>
...
from premium import gross_annual_premium, periodic_premium, endowment_annual_premium as net_endow_ap

if use_ml:
    lt = get_ml_life_table(gender_code, ml_params)
    lt['qx'] = lt['qx'] * risk_factor
else:
    lt = cached_life_table(st.session_state.table_path, st.session_state.table_col, gender_code, risk_factor)
...
net = net_endow_ap(lt, age, sum_insured, term, rate, claim_accel)
...
param_items = [
    ...
    (t('risk_label'), risk_label), (t('payment_label'), pay_label),
]
render_param_chips(param_items)

render_metric_row([
    {
        'label': f'{t("per_payment")} ({freq_label})',
        'value': f'¥{payment:,.0f}',
        'help': t('help_per_payment', gap=f'¥{gap:,.0f}'),
    },
    {'label': t('gross_annual'), 'value': f'¥{gap:,.0f}', 'help': 'Annual premium with expenses'},
    {
        'label': t('net_premium'),
        'value': f'¥{net:,.0f}',
        'help': t('net_premium_help'),
        'delta': f'¥{gap - net:,.0f}',
        'delta_color': 'off',
    },
])
st.caption(t('caption_summary', freq=freq_label, term=term, extra=f'¥{gap-net:,.0f}', sum_insured=f'{sum_insured:,}'))
